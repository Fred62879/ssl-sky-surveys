...
class Trainer():
  ...
  def __init__(self, params):
    ...
    logging.info(f"training #batch is: {num_batches}.")

    self.model = models.resnet.resnet50(
      num_channels=params.num_channels, num_classes=params.num_classes).to(self.device)
  ...
  def train(self):
    ...
    for epoch in range(self.startEpoch, self.params.max_epochs):
      ...
      if self.params.log_to_screen:
        ...
        logging.info('train data time={}, train time={}, valid step time={}, train acc1={}, valid acc1={}'.format(data_time, tr_time, valid_time, train_logs['acc1'], valid_logs['acc1']))
  ...
  def train_one_epoch(self):
    ...
    for i, data in enumerate(self.train_data_loader, 0):
      self.iters += 1
      logging.info(f"[epoch/batch] {self.epoch}/{i + 1}")
      ...
      with torch.cuda.amp.autocast(self.params.amp):
        ...
        # logging.info(f"forward pass takes {elapsed}s.")

        # print(outputs.shape, specz_bin)
        # assert 0
        loss = self.criterion(outputs, specz_bin)

        mx, preds = outputs.max(1)
        print(preds, mx)
  ...
  def restore_checkpoint(self, checkpoint_path):
    ...
    # print('**********')
    # for n in checkpoint['state_dict']: print(n)
    # assert 0
